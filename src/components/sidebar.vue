      ...
      <div id="sidebar-menu">
        <ul class="sidebar-links custom-scrollbar" id="myDIV" :style="[
          ...
          <li v-for="(menuItem, index) in menuItems" :key="index" :class="{
            ...
            <!-- Link -->
            <a href="javascript:void(0)" @click="loadComponent(menuItem)" class="sidebar-link sidebar-title"
            ...
            <ul class="sidebar-submenu" v-if="menuItem.children" :class="{ 'menu-open': menuItem.active }">
              <li v-for="(childrenItem, index) in menuItem.children" :key="index"
                ...
                <!-- Sub -->
                <a class="submenu-title" href="javascript:void(0)" v-if="childrenItem.type == 'sub'"
                ...
                <!-- Link -->
                <router-link class="submenu-title" :to="childrenItem.path"
                  :event="childrenItem.disableClick ? '' : 'click'" v-if="childrenItem.type == 'link'">
                  {{ childrenItem[getLocaleKey("title")] }}
                  <label :class="
                    'badge badge-' + childrenItem.badgeType + ' pull-right'
                  " v-if="childrenItem.badgeType">{{ childrenItem.badgeValue }}</label>
                ...
                <!-- 3rd Level Menu -->
                <ul class="nav-sub-childmenu submenu-content" v-if="childrenItem.children && childrenItem.children.length"
                  :class="{ opensubchild: childrenItem.active }">
                  <li v-for="(childrenSubItem, index) in childrenItem.children" :key="index">
                    <!-- Link -->
                    <router-link :to="childrenSubItem.path" v-if="childrenSubItem.type == 'link'"
                      :event="childrenSubItem.disableClick ? '' : 'click'" router-link-exact-active>
...
export default {
  ...
  methods: {
    ...
    fetchRoles() {
      ...
      rolesPermissionsService.fetchRolesList().then(res => {
        if (res.data.status) {
